...
public class Spiral : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        tria = new int[points * pointsOfCircle * 6 - pointsOfCircle * 3];

        
        for (int i = 0; i < points; i++)
        {
            vert[i] = new Vector3(
                Mathf.Cos(360f * spiralCount / points * i * Mathf.Deg2Rad),
                i * heightPower,
                Mathf.Sin(360f * spiralCount/ points * i * Mathf.Deg2Rad)  
                );
        }
        ...
        for (int j = 0; j < points; j++)
        {
            for (int i = 0; i < pointsOfCircle; i++)
            {
                ...
                vert[points + i + pointsOfCircle * j] = Matrix4x4.Rotate(Quaternion.Euler(0, -360f * spiralCount/points * j, 0)).MultiplyVector(vert[points + i + pointsOfCircle * j]);
